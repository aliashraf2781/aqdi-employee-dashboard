...
export const RELIABLE_ORDERS_QUERY_KEY = "reliableOrders";
export const RELIABLE_ORDERS_API = "/admin/orders/status/9";

export default function ReliableOrdersWrapper() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [RELIABLE_ORDERS_QUERY_KEY, debouncedSearchQuery, currentPage],
    queryFn: () => {
      let url = `${RELIABLE_ORDERS_API}?page=${currentPage}`;
  ...
  const exportConfig = useMemo(
    () => ({
      ...
      onExport: (rows) =>
        exportOrdersToExcel(rows, { filename: "طلبات-موثقة", showStatusColumn: true }),
    }),
    [selectedOrders]
  );
  ...
  return (
    <div className="flex flex-col gap-6 p-6 min-h-screen" dir="rtl">
      <Header
        ...
        firstURL="/"
        second="طلب موثق"
        secondURL="/home/reliable-orders"
      />
      ...
      <div className="flex flex-col gap-6 mt-4 relative z-10">
        <OrdersToolbar
          ...
          showAddButtons
          queryKeys={[RELIABLE_ORDERS_QUERY_KEY]}
      ...
      <OrdersTable
        ...
        showStatusColumn
        showChangeStatus
        queryKey={[RELIABLE_ORDERS_QUERY_KEY]}
