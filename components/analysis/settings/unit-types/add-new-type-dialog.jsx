...
export default function AddNewTypeDialog() {
  ...
  function addNewUnit() {
    return axiosInstance.post("/admin/unit-types", {
      name_ar: unitName,
      contract_type: unitType,
    })
  }

  const {mutate: addNewUnitMutate, isPending: addNewUnitPending} = useMutation({
    mutationFn: addNewUnit,
    onSuccess: (res) => {
      ...
      <DialogContent closeButton={false} className="max-w-3xl">
        <DialogHeader>
          <div className='flex items-center justify-between  border-b pb-6'>
            ...
            <h2 className='text-xl font-bold'>إضافة نوع جديد</h2>
          ...
          <div dir='rtl' className='space-y-4 text-right'>
            ...
            {/* زر الإضافة */}
            <Button
              disabled={addNewUnitPending}
              onClick={handleSubmit}
              className="mx-auto block  h-12 bg-brand-hover"
            >
              {addNewUnitPending ? <Loader2 className='animate-spin'/> : "إضافة"}
            </Button>
          </div>
