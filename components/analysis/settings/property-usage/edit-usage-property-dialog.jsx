...
export default function EditUsagePropertyDialog({unit}) {
  ...
  function editUnit() {
    return axiosInstance.post(`/admin/real-estate-usages/${unit.id}`, {
      name_ar: unitName,
  ...
  const { mutate: editUnitMutate, isPending: editUnitPending } = useMutation({
    ...
    onError: (error) => {
      toast.error(error?.response?.data?.message || "حدث خطأ أثناء تعديل استخدام العقار");
    }
  })
      ...
      <DialogContent closeButton={false} className="max-w-3xl">
        <DialogHeader>
          <div className='flex items-center justify-between  border-b pb-6'>
            ...
            <h2 className='text-xl font-bold'>تعديل استخدام العقار</h2>
