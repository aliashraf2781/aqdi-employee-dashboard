...
export default function UnitTypesPage() {
  ...
  function getUnitTypes() {
    return axiosInstance(`/admin/unit-types?contract_type=${activeTab}`);
  }

  const { data: unitTypes, isLoading: unitTypesLoading } = useQuery({
    queryKey: ["unit-types", activeTab],
    queryFn: getUnitTypes,
  })

  const data = unitTypes?.data?.data?.items;


  // delete unit type
  function deleteUnitType(id) {
    return axiosInstance.post(`/admin/unit-types/${id}/delete`);
  }
